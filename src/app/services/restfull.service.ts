...
export class RestfullService {
  ...
  private FileConfig = AppconfigService.Settings;
  
  Module = {
    account: "account/"+ this.FileConfig.REST_API_VERSION,
    shop: "shop"+ this.FileConfig.REST_API_VERSION,
    product: "product"+ this.FileConfig.REST_API_VERSION,
    point: "point"+ this.FileConfig.REST_API_VERSION
  }
  ...
  public postHTTP(Module: string,URl: string, PostData: any ){
    ...
    return this.httpClient.post(this.REST_API_SERVER+Module+URl, PostData, headers).pipe(catchError(this.handleError));
  }

  public postHTTPForm(Module: string,URl: string, PostData: any ){
    // X-FORM : 'Content-Type': 'application/x-www-form-urlencoded'
    // X-JSON : "Accept": 'application/json'
    let headers =  {headers: new  HttpHeaders({ 'Accept': 'application/x-www-form-urlencoded', 'Content-Type': 'application/json'})};
    return this.httpClient.post(this.REST_API_SERVER+Module+URl, PostData, headers).pipe(catchError(this.handleError));
  }
